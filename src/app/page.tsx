...
export default function Dashboard() {
	...
	const task = useQuery(api.tasks.getTask, activeTaskId ? { taskId: activeTaskId } : "skip");
			...
			<div className="max-w-5xl mx-auto space-y-8">
				...
				<header className="flex items-center justify-between border-b border-zinc-800 pb-6">
					<div>
						...
						<p className="text-zinc-500 mt-1">Local OpenCode Worker Dashboard</p>
					</div>
				...
				<section className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6 shadow-xl backdrop-blur-sm">
					<form onSubmit={handleRunTask} className="flex gap-4">
								...
								<div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl overflow-hidden shadow-xl">
									...
									<div className="px-6 py-4 bg-zinc-950/50 border-b border-zinc-800 flex items-center justify-between">
										<div className="flex items-center gap-3">
												<span className="text-sm font-medium text-zinc-400">ID: {task._id.slice(-8)}</span>
												<div className={`px-2 py-0.5 rounded-md text-[10px] uppercase font-bold tracking-wider ${
											...
											<div key={idx} className="flex gap-4 group">
												<div className="flex flex-col items-center">
													<div className={`w-8 h-8 rounded-full flex items-center justify-center border ${
														step.type === 'tool_call' ? 'bg-blue-500/10 border-blue-500/30 text-blue-400' :
														step.type === 'tool_result' ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' :
														step.type === 'final' ? 'bg-amber-500/10 border-amber-500/30 text-amber-400' :
														'bg-zinc-800 border-zinc-700 text-zinc-400'
													}`}>
														{step.type === 'tool_call' ? <Wrench className="w-4 h-4" /> :
														 step.type === 'tool_result' ? <CheckCircle2 className="w-4 h-4" /> :
														 step.type === 'final' ? <Terminal className="w-4 h-4" /> :
														 <Info className="w-4 h-4" />}
													</div>
													{idx < task.steps.length - 1 && <div className="w-px flex-1 bg-zinc-800 my-1" />}
												</div>
												<div className="flex-1 pt-1">
													<p className="text-zinc-200 font-medium leading-tight">{step.message}</p>
								...
								{recentTasks?.map((t: any) => (
									...
									>
										...
										<ChevronRight className={`w-4 h-4 transition-transform ${
												activeTaskId === t._id ? 'text-blue-400 translate-x-1' : 'text-zinc-600 group-hover:text-zinc-400'
										}`} />
									</button>
								))}
							</div>
